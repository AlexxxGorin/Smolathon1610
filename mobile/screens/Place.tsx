...
import like from '../assets/fuck.png'
...
const Place = ({ route, navigation }) => {
      ...
      useEffect(() => {
        ...
        const fetchUserData = async () => {
          ...
          try {
            ...
            if (user !== null) {
              const response = await axios.get(`http://195.80.50.92:8080/user/get?id=${user[1]}`);
              // console.log("bruh", JSON.parse(response['request']['response']))
              setUserData(JSON.parse(response['request']['response']));
              for (let i = 0; i <JSON.parse(response['request']['response'])["places"].length; i++){
                if (JSON.parse(response['request']['response'])["places"][i]["id"] == otherParam["id"].toString())
                setLiked(true);
              }
    ...
    //   const navigation = props.navigation
      ...
      function handleLike() {
        console.log('allo', userData)
        var newLiked = userData['places'];
        if (liked){
          setLiked(false)
          newLiked.splice(userData['places'].indexOf(otherParam["id"].toString()), 1)
        }
        else {
          setLiked(true)
          newLiked.push(otherParam["id"].toString())
        }
        axios.put(`http://195.80.50.92:8080/user/like_place?place_id=${otherParam["id"]}&user_id=${userData["id"]}`)
        // dispatch(setProfile({
        //   'id': profile['id'],
        //   'login': profile['login'],
      ...
      if (features !== null && tags !== null) {
        ...
        newTags = newTags.sort((a: any, b: any) => parseInt(weights[b["name"]]) - parseInt(weights[a["name"]]))
    ...
    //     }
    // }
    const STORIESDATA = Object.keys(photos).map(key => {
      return {
        id: Object.keys(photos).indexOf(key),
        title: key,
        preview: photos[key][0],
        images: photos[key]
      }
    })
    ...
    //   ];
      ...
      const renderTags = ({ item }) => {
        if (item.id == 0 && Object.keys(features).includes('Цены')) {
          if (Object.keys(features).includes('Средний счёт')){
            return (
                <TouchableOpacity onPress={() => setClickedBill(!clickedBill)} activeOpacity={0.6}><View style={styles.tag}><Text style={styles.tagText}>{clickedBill == true ? '💸  ' + features['Средний счёт'][0] : '💸  ' + features['Цены'][0].charAt(0).toUpperCase() + features['Цены'][0].slice(1)}</Text></View></TouchableOpacity>
            )
          }
            ...
            return (
                <TouchableOpacity onPress={() => setClickedBill(!clickedBill)} activeOpacity={0.6}><View style={styles.tag}><Text style={styles.tagText}>{features['Цены'][0].charAt(0).toUpperCase() + features['Цены'][0].slice(1)}</Text></View></TouchableOpacity>
            )
          }
          
        }
        else if (item.id == 0 && Object.keys(features).includes('Средний счёт')) {
          return (
            <TouchableOpacity onPress={() => setClickedBill(!clickedBill)} activeOpacity={0.6}><View style={styles.tag}><Text style={styles.tagText}>{'💸  ' + features['Средний счёт'][0]}</Text></View></TouchableOpacity>
            )
        }
        const flags = {
          'европейская': '🇪🇺',
        ...
        if (item.id > 1 && newTags[item.id - 2]["name"] !== 'string') {
          return (
      ...
      return (
        <View style={{ flex: 1 }}>
          <ScrollView nestedScrollEnabled={true} style={{ width: '100%', backgroundColor: '#033', }} contentContainerStyle={{ flexGrow: 1, paddingBottom: 0 }}>
            <TouchableOpacity onPress={handleBack} style={{ zIndex: 2, top: Platform.OS == 'ios' ? 36 : 6, width: 80 }}><Image style={{ width: 21, height: 50, marginLeft: '25%', position: 'relative', zIndex: 2 }} source={Back}></Image></TouchableOpacity>
            ...
            {/* LIKE AND SHARE */}
            <TouchableOpacity onPress={() => handleLike()} style={{ zIndex: 2 }}><View><Image style={{ zIndex: 1, left: '83%', top: 15, position: 'absolute', width: 25, height: 25 }} source={liked ? likeActive : like} /></View></TouchableOpacity>
            ...
            {/* INFO */}
            <Text style={{ position: 'relative', color: '#FFF', fontSize: name.length > 10 ? name.length > 15 ? 18 : 20 : 22, fontWeight: '700', marginTop: 10, left: "8%", zIndex: 0, maxWidth: '75%', maxHeight: 63, marginBottom: name.length > 10 ? 15 : 0 }}>{name}</Text>
            <Text style={{ position: 'relative', color: "#FFF", marginTop: 15, fontSize: 12, width: '70%', left: "8%" }}>{desc}</Text>
    
            {/* TAGS */}
            <TouchableOpacity onPress={() => Linking.openURL(link)} style={{ zIndex: 2 }} >
              <View style={{ zIndex: 1, marginTop: 15, position: 'relative', backgroundColor: '#1E1E1E', left: '5%', alignSelf: 'flex-start', padding: 7, maxHeight: 34, paddingRight: 14, borderRadius: 15 }}>
            ...
            <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
              <View style={{ width: 'max-content', left: '16%', paddingRight: 40, marginTop: 20 }}>
                <FlatList
                  // numColumns={3}
