...
const PlaceHome = (props:any) => {
    ...
    const [userData, setUserData] = useState('');
      ...
      if (props.photos["Внутри"] !== undefined)
      var img = props.photos["Внутри"][0]
      else if (props.photos["Все"] !== undefined)
      var img = props.photos["Все"][0]
      else if (props.photos["Снаружи"] !== undefined)
      var img = props.photos["Снаружи"][0]
      else if (props.photos["Услуги"] !== undefined)
      var img = props.photos["Услуги"][0]
      else var img = null;
      ...
      const id = props.id
      if (Object.keys(features).includes('Кухня'))
      var cusine = features["Кухня"];
      else var cusine = undefined;
      ...
      if (features !== null && tags !== null) {
        ...
        const renderTags = ({ item }: any) => {
            ...
            if (emojis[newTags[item.id]] != '' && newTags[item.id]["name"] !== 'string') {
                return (
        ...
        const [cusineNum, setCusineNum] = useState(cusine !== undefined ? cusine[0] === 'европейская' && ['домашняя', 'смешанная', 'вегетерианская', 'веганская', 'шашлык', 'славянских народов', 'континентальная', 'халяльная'].includes(cusine[1]) != true && cusine.length > 1 ? 1 : 0: 0);
        ...
        if (Object.keys(features).includes('Цены')) {
        ...
        else if (Object.keys(features).includes('Средний счёт')) {
          return (
          ...
          <View style={{ width: 'auto', height: '100%' }}>
            ...
            <TouchableOpacity activeOpacity={1}  onPress={() => handleProfile()}  style={{ zIndex: 2 }}>
              <View style={styles.infoContainer}>
                ...
                {time !== null ? 
                <Text style={styles.time}>{time['Понедельник']}</Text> : <View/>}
                </View>
            </TouchableOpacity>
